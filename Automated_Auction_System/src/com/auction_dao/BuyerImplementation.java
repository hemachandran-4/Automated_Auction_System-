...
public class BuyerImplementation implements BuyerInterface{
		...
		public List<ProductFull> viewAllProdcut() throws ProductException {
			List<ProductFull> plist=new ArrayList<>();
			try(Connection conn=DBUtil.provideConnection()){
				...
				while(rs.next()) {
					...
					ProductFull p1=new ProductFull(product_id,product_name, seller_id, product_price, disputed, product_added_date, product_time_limit, availability, product_minimum_bid, sold, buyer_id);
					plist.add(p1);
				}
			}
			catch(SQLException e) {
